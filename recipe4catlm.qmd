...
penguins |> 
  ...
  aes(x = bill_depth_mm,
      ...
      cat = species) + 
  geom_point() + 
  geom_point(stat = StatLmCat) +
  geom_line(stat = StatLmCat) + 
...
Below we use the new function `geom_lm_cat()`, constrasting it to `geom_smooth()`, with parallel and unparallel slopes respectively.
...
### Step 1: Write compute functions. Test.
...
# Define.
StatCatResiduals <- ggproto(`_class` = "____",
                     `_inherit` = Stat,
                     compute_panel = ____,
                     required_aes = c("x", "____", "cat"))
...
penguins_clean |> 
  ...
  geom_point() +
  geom_point(stat = StatCatFitted, color = "coral")
  geom_segment(stat = ____, color = "coral")
...
stat_cat_fitted <- function (mapping = NULL, data = NULL, geom = "point", position = "identity", 
    ..., show.legend = NA, inherit.aes = TRUE){
    layer(data = data, mapping = mapping, stat = StatCatFitted, 
        geom = geom, position = position, show.legend = show.legend, 
        inherit.aes = inherit.aes, params = rlang::list2(na.rm = FALSE, 
            ...))
}
...
# Define.
stat_cat_residuals <- function (mapping = NULL, data = NULL, geom = "segment", position = "identity", 
    ..., show.legend = NA, inherit.aes = TRUE){
