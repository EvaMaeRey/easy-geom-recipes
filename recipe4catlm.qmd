...
::: {.callout-tip collapse="true"}
## Pro tip. 🎉 Use `stat_cat_lm <- make_constructor(StatCatLm, geom = "line")` from the next ggplot2 release.
...
# Be aware that verbatim aliasing as shown above is a bit of a shortcut.  (🎉 but we can use make_constructor in the next ggplot2 release, just about as easy and more conventional: `geom_cat_lm <- make_constructor(GeomLine, stat = "cat_lm")`)
...
geom_cat_lm <- function(mapping = NULL, data = NULL, 
                         stat = "cat_lm", position = "identity", 
                         ..., show.legend = NA, inherit.aes = TRUE) 
{
    layer(data = data, mapping = mapping, stat = stat, 
        geom = GeomLine, position = position, show.legend = show.legend, 
        inherit.aes = inherit.aes, params = rlang::list2(na.rm = FALSE, 
            ...))
}
```

However, because it is unexpected to use `geom_cat_lm()` with a Stat other than StatIndex (doing so would remove the index-ness) we think that the verbatim aliasing is a reasonable, time and code saving getting-started approach.  
...
Below we use the new function `geom_cat_lm()`, constrasting it to `geom_smooth()`, with parallel and unparallel slopes respectively.
...
Using the `geom_cat_lm` Recipe #3 as a reference, try to create a `geom_cat_fitted()` and `geom_cat_residuals()` that draws a segment between observed and fitted values for a linear model. 
