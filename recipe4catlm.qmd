...
Because users are more accustom to using layers that have the 'geom' prefix, you might also define a user-facing `geom function` with almost identical properties using `make_constructor()`. 
...
```{webr}
#| exercise: geom_means
# Define stastat_cat_fitted
stat_cat_fitted <- make_constructor(StatCatFitted, geom = ____)

# Define stat_cat_residuals
stat_cat_residuals <- make_constructor(____, geom = "segment")

# Define geom_cat_fitted
geom_cat_fitted <- make_constructor(GeomPoint, stat = "cat_fitted")

# Define geom_cat_residuals
geom_cat_residuals <- make_constructor(____, stat = "cat_residuals")
