...
Creating a new geom_\*() or stat_\*() function is often motivated when much precomputation using base ggplot2 is required otherwise. 

By using Stat extension, you can pre-define computation to be performed on-the-fly, *within* the plotting pipeline. 

Let's get started.

In this exercise, we'll think about a way to add a point at the medians x and y, defining the new extension function `geom_medians()` which could be used as follows:
...
## Express alternative for user-facing functions (Step 3).
