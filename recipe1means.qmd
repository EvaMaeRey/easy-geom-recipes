...
```{r, echo = F, code = readLines("make_constructor.R")}
...
```{r error = T, eval = T}
stat_medians <- make_constructor(StatMedians, GeomPoint)
geom_medians <- make_constructor(GeomPoint, StatMedians)
geom_medians_label <- make_constructor(GeomLabel, StatMedians)
