...
Because plotnine users, like ggplot2 users, may be more accustomed to using layers that have the `geom_` prefix, you might also define a `geom_` function with almost the same properties as the `stat_`.  Here we'll use `make_constructor()` for convenience.
