...
2. **that the compute function assumes that variables x and y are present.**  These are aesthetic variables names, relevant for building the plot, generally not found in the raw data inputs for ggplots.
:::
...
**that we prepare the data to have columns with names x and y before testing `compute_group_medians`.** Computation will fail if they names x and y are not present given the function definition.  Internally in a plot, columns are renamed when mapping aesthetics: `aes(x = bill_depth, y = bill_length)`.
:::
...
You may be thinking, defining a new stat_\*() function is a mouthful that's probably hard to get right from memory.  So you might use `stat_identity()`'s definition as scaffolding to write your own layer. i.e:
