...
:::   {.callout-tip collapse="true"}
# Pro tip: use layer() function to test ...

Instead of using a `geom_*()` function, you might prefer to use the more flexible `layer()` function in your testing step.  This is in fact, sometimes necessary, when user facing functions contain no `stat` argument like `geom_vline()` for example.
