...
An alternate and recommended route is to use the `make_constructor()` function, newly available in the ggplot2 v4.0.0 release, to write the scaffolding code for you, 

```{r, eval = F}
stat_medians <- make_constructor(StatMedians, geom = "point")
```

You can then check that this specifies the function with the boilerplate code as we have done above. 
...
Because ggplot2 users may be more accustomed to using layers that have the `geom_` prefix, you might also define a `geom_` function with almost the same properties as the `stat_`.  Here we'll use `make_constructor()` for convenience. 
...
Below, we'll have a look at the definition of our `geom_*()` function that `make_constructor()` helps us write. We see that it's nearly identical to the `stat_*()` definition.  But following ggplot2 conventions, our `geom_`-prefixed function will have a fixed `Geom` and the flexible `Stat`. 
...
#| exercise: stat_means
# Define stat_means
stat_means <- make_constructor(____, geom = "point")

# Define geom_means
geom_means <- make_constructor(GeomPoint, stat = ____)
