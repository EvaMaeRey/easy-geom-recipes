...
layer_data(
  plot = last_plot(),
  i = 2
) # layer 2, the computed means, is of interest
```
:::
...
stat_medians <- function(mapping = NULL, data = NULL,
  ...
  layer(
    data = data, mapping = mapping, stat = StatMedians,
...
geom_medians_label <- function(...) {
  geom_label(stat = StatMedians, ...)
}
...
penguins_means <- penguins_clean %>% 
  summarise(
    mean_bill_depth_mm = ____(bill_depth_mm),
...
stat_means <- function (mapping = NULL, data = NULL,
                        ...
                        ..., show.legend = NA, inherit.aes = TRUE){
    layer(
      data = data, mapping = mapping, stat = ____, 
      geom = geom, position = position, show.legend = show.legend, 
