...
```{r cars}
plot_multi_responses <- function(data, var){
data %>%
  pull({{var}}) %>%
  paste(collapse = ";") %>%
  str_split_1(";") %>%
...
# checkbox
plot_multi_responses(survey, q09)
ggplot(survey) + 
  aes(x = q09) + 
  geom_bar() + 
  labs(title = q_long[9])
...
# checkbox
plot_multi_responses(survey, q18)
...
toy_survey %>% 
  ...
  aes(x = x, y = y) + 
  geom_col() + 
  labs(title = "Number of people indicating each feeling")
