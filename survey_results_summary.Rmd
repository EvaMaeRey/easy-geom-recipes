...
```{r, eval = T}
compute_panel_multi_response <- function(data = survey, scales, cat_levels = q11_levels){
  
   data = data.frame(responses = survey$q11)

  data %>%
    summarise(responses = paste(responses, collapse = ";")) %>% 
    mutate(response = str_split(responses, ";")) %>% 
    select(-responses) %>% 
    unnest(response) %>% 
    filter(response != "NA") %>% 
    count(response) %>%
    mutate(x = factor(response, cat_levels) %>% as.numeric %>% as.double()) %>%
...
StatMulticat <- ggplot2::ggproto(`_class` = "StatMulticat",
                        ...
                        # required_aes = "response"
                        )



ggplot(data = survey %>% select(q11) %>% remove_missing()) + 
  aes(response = q11) + 
  layer(geom = "bar", stat = StatMulticat, position = "identity") + 
  scale_x_continuous(breaks = 1:length(q11_levels), labels = q11_levels) + 
  coord_flip()
```
