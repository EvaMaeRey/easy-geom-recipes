...
```{r , include=FALSE}
knitr::opts_chunk$set(echo = T, eval = T, fig.width = 8, fig.height = 5.5, out.width = "85%", warning = F)
```
...
compute_panel_multi_response <- function(data, 
  ...
  data %>%
    summarise(responses = paste0(responses, collapse = ";")) %>% 
...
data.frame(liked_fruit = c("banana;apple;pear", "apple;pear", "banana;pear")) %>% 
  ggplot() +  
  aes(responses = liked_fruit) + 
...
data.frame(liked_fruit = 
             c("banana;apple;pear", 
               "apple;pear", 
               "banana;pear")) %>%
  select(responses = liked_fruit) %>% 
