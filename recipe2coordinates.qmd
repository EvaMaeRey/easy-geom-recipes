...
In this exercise, we'll think about a way to annotate the row number at x and y, defining the new extension function `geom_index()` and then you'll be prompted to define `geom_coordinates()` based on what you've learned.
...
We'll use the tidyverse's tibble to convert the cars data frame into a tibble.  This is mostly so we'll have nicer, redacted print of the data. 
...
# Define compute.
compute_group_index <- function(data, ...){ 
  data |> 
...
2. **that the compute function adds a column of data called 'label'**  We assume that x and y will be present in our data as the user will declare them.  However, the `label` aes, which is required for Geoms like Text and Label will be computed *internally* in our layer.
:::
...
**that we prepare the data to have columns with names x and y before testing `compute_group_index`.** We just do this to be more consistent with how data will look internally in a ggplot --- columns are renamed when mapping aesthetics, e.g. `aes(x = speed, y = dist)`.  
:::
...
**that some indices change with color mapping.**  Because compute is defined by `compute_group`, the row_number() computation and therefore index is within group. 
:::
...
cars |> 
  ...
  aes(x = speed, y = dist) + 
  geom_point() +
  geom_text(stat = ____, hjust = 1, vjust = 1)
