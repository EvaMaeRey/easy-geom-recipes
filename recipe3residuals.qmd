...
Welcome to 'Easy `geom` recipes' Recipe #3: creating `geom_lm_cat()` and `geom_lm_cat_residuals()`.
...
::: {.callout-tip collapse="true"}
## Pro tip.  Use `make_constructor` from the next release
...
::: {.callout-tip collapse="true"}
## Pro tip.  Use `make_constructor` from the next release, which will be more consistent with most geom_*() formulations
...
Using the `geom_lm_cat` Recipe #3 as a reference, try to create a `stat_lm_residuals()` and convenience `geom_fitted()` that draws a segment between observed and fitted values for a linear model. 
