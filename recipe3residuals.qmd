...
StatXMean <- ggproto("StatXMean", Stat,
                     ...
                     default_aes = aes(label = round(after_stat(mean_x), 2)))

cars|>
  ggplot() + 
  aes(speed, xend = lag(speed),  dist, yend = lag(dist)) +
  geom_point() + 
  layer(geom = GeomVline, stat = StatXMean, position = "identity") +
  geom_label(stat = StatXMean) + 
  geom_segment(lineend = "round") + 
  geom_segment(lineend = "round", aes(xend = speed, yend = dist),
               linewidth = 7)
